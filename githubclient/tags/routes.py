import os
import json
from flask import render_template, url_for, flash, redirect, request, Blueprint, Response, current_app, abort
from flask_login import login_user, current_user, logout_user, login_required
from githubclient import db, bcrypt
from flask import Blueprint
from flask_dance.contrib.github import make_github_blueprint, github
from githubclient.tags.forms import TagsForm
from githubclient.models import Tags, Repository
...
def list():
    ...
    if (github_account_info and 'id' in github_account_info):
        tags = Tags.query.filter_by(id_github_account=github_account_info['id']).order_by(Tags.name.asc()).paginate(page=page, per_page=rows_per_page) 
...
def edit(tag_id):
    ...
    tag = Tags.query.filter_by(id=tag_id, id_github_account=github_account_info['id']).first_or_404() 
    form = TagsForm()
...
def delete():
    ...
    tag = Tags.query.filter_by(id=tag_id, id_github_account=id_github_account).first_or_404()
...
def getTags():
    ...
    tags_info = {}
    tags = Tags.query.filter_by(id_github_account=id_github_account).order_by(Tags.name.asc()).all()
