    ...
    name = StringField(
        ...
        validators=[
            ...
            Length(
                min=2,
                max=40,
                message="A tag deve conter pelo menos %(min)d e no máximo %(max)d caracteres",
            ),
        ],
    )
        ...
        if github.authorized:
            ...
            if github_account_info.ok:
                ...
                tag = Tags.query.filter_by(
                    name=name.data, id_github_account=github_account_info_json["id"]
                ).first()
                if tag and name.data != self.origin_name.data:
                    raise ValidationError(
                        "Já existe uma tag com este nome, por favor escolha outro."
                    )
