...
def test_tags_add_post(monkeypatch):
    ...
    with app.test_client() as client:
        response = client.post(
            os.getenv("BASE_URL") + "/tags/add", headers=headers, data=payload
        )
        assert response.status_code == 200
...
def test_tags_delete_post(monkeypatch):
    ...
    with app.test_client() as client:
        response = client.post(
            os.getenv("BASE_URL") + "/tags/delete", headers=headers, data=payload
        )
        assert response.status_code == 404
...
def test_relationship(monkeypatch):
    ...
    with app.test_client() as client:
        response = client.post(
            os.getenv("BASE_URL") + "/tags/relationship", headers=headers, data=payload
        )
        assert response.status_code == 404
...
def test_remove_relationship(monkeypatch):
    ...
    with app.test_client() as client:
        response = client.post(
            os.getenv("BASE_URL") + "/tags/removeRelationship",
            headers=headers,
            data=payload,
        )
        assert response.status_code == 404
